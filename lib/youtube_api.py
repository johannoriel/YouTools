...
class YoutubeAPI:
    ...
    # In youtube_api.py, replace the get_transcript method
    def get_transcript(self, video_id: str, language: str = "fr") -> tuple[str, str]:
        """
        Retrieve transcript for a YouTube video, trying multiple methods.
        ...
        for lang in language_fallbacks:
            try:
                ...
                print(f"YouTubeTranscriptApi: No transcript available for lang {lang}: {str(e)}")
                continue
            except Exception as e:
        ...
        try:
            ...
            self.track_quota_usage(50)  # Captions list costs 50 quota units

            captions = caption_response.get("items", [])
            if not captions:
                print(f"YouTube API: No captions found for video {video_id}")
                return "", "none"

            # Prioritize French captions, then auto-generated
            target_caption = None
            for caption in captions:
                caption_lang = caption["snippet"]["language"]
                print(f"Found caption track: {caption_lang}, isAutoGenerated: {caption['snippet'].get('isAutoGenerated', False)}")
                if caption_lang.startswith("fr"):
                    target_caption = caption
                    break
                elif caption["snippet"].get("isAutoGenerated", False) and not target_caption:
                    target_caption = caption

            if not target_caption:
                print(f"YouTube API: No suitable caption track found for video {video_id}")
                return "", "none"
            ...
            self.track_quota_usage(200)  # Caption download costs 200 quota units

            # Parse SRT content
            full_transcript = self._parse_srt(caption_content.decode("utf-8"))
            print(f"Transcript retrieved via YouTube Data API for lang {target_caption['snippet']['language']}")
            return full_transcript, target_caption["snippet"]["language"]

        except HttpError as e:
            if e.resp.status == 403:
                print(f"YouTube API: Captions disabled or not available for video {video_id}")
            else:
                print(f"YouTube API Error (get_transcript): {str(e)}")
            return "", "none"
        except Exception as e:
            print(f"Unexpected Error (get_transcript): {str(e)}")
            return "", "none"
